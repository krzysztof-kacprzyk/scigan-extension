            ...
            for treatment in range(self.num_treatments):
                ...
                for index in range(self.num_dosage_samples):
                    ...
                    treatment_layer_4 = tf.layers.dense(treatment_layer_3, self.h_dim, activation=tf.nn.elu,
                                                        name='treatment_layer_4_%s' % str(treatment),
                                                        reuse=tf.AUTO_REUSE)
                    treatment_layer_5 = tf.layers.dense(treatment_layer_4, self.h_dim, activation=tf.nn.elu,
                                                        name='treatment_layer_5_%s' % str(treatment),
                                                        reuse=tf.AUTO_REUSE)

                    treatment_dosage_output = tf.layers.dense(treatment_layer_5, 1, activation=None,
                                                              name='treatment_output_%s' % str(treatment),
                                                              reuse=tf.AUTO_REUSE)
        ...
        with tf.variable_scope('inference', reuse=tf.AUTO_REUSE):
            ...
            for treatment in range(self.num_treatments):
                ...
                for index in range(self.num_dosage_samples):
                    ...
                    treatment_layer_6 = tf.layers.dense(treatment_layer_5, self.h_dim, activation=tf.nn.elu,
                                                        name='treatment_layer_6_%s' % str(treatment),
                                                        reuse=tf.AUTO_REUSE)

                    treatment_dosage_output = tf.layers.dense(treatment_layer_6, 1, activation=None,
                                                              name='treatment_output_%s' % str(treatment),
                                                              reuse=tf.AUTO_REUSE)
...
class SCIGAN_deep_4_Model:
    ...
    def generator(self, x, y, t, d, z, treatment_dosage_samples):
        with tf.variable_scope('generator', reuse=tf.AUTO_REUSE):
            ...
            for treatment in range(self.num_treatments):
                ...
                treatment_layer_5 = tf.layers.dense(treatment_layer_4, self.h_dim, activation=tf.nn.elu,
                                                    name='treatment_layer_5_%s' % str(treatment), reuse=tf.AUTO_REUSE)
                                                    
                treatment_layer_6 = tf.layers.dense(treatment_layer_5, self.h_dim, activation=tf.nn.elu,
                                                    name='treatment_layer_6_%s' % str(treatment), reuse=tf.AUTO_REUSE)
    ...
    def treatment_discriminator(self, x, y, treatment_dosage_samples, treatment_dosage_mask,
        ...
        with tf.variable_scope('treatment_discriminator', reuse=tf.AUTO_REUSE):
            ...
            patient_features_representation_2 = tf.layers.dense(patient_features_representation, self.h_dim, activation=tf.nn.elu)
            ...
            D_treatment_representations = tf.concat(list(D_treatment_outcomes.values()), axis=-1)
            D_shared_representation = tf.concat([D_treatment_representations, patient_features_representation_2], axis=-1)

            D_treatment_rep_hidden = tf.layers.dense(D_shared_representation, self.h_dim, activation=tf.nn.elu,
                                                     name='rep_all',
                                                     reuse=tf.AUTO_REUSE)
            D_treatment_rep_hidden_2 = tf.layers.dense(D_treatment_rep_hidden, self.h_dim, activation=tf.nn.elu,
                                                     name='rep_all_2',
                                                     reuse=tf.AUTO_REUSE)

            D_treatment_logits = tf.layers.dense(D_treatment_rep_hidden_2, self.num_treatments, activation=None,
                                                 name='output_all',
                                                 reuse=tf.AUTO_REUSE)
